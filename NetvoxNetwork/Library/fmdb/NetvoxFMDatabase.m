...
@interface NetvoxFMDatabase ()

- (NetvoxFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation NetvoxFMDatabase
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    NetvoxFMDatabase *self = (__bridge NetvoxFMDatabase*)f;
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        NetvoxFMResultSet *rs = (NetvoxFMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(NetvoxFMResultSet *)resultSet {
...
- (NetvoxFMStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(NetvoxFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(NetvoxFMStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    NetvoxFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"NetvoxFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
- (NetvoxFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (NetvoxFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    NetvoxFMStatement *statement  = 0x00;
    NetvoxFMResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[NetvoxFMStatement alloc] init];
    ...
    rs = [NetvoxFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (NetvoxFMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (NetvoxFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (NetvoxFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (NetvoxFMResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    NetvoxFMStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[NetvoxFMStatement alloc] init];
...
int NetvoxFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int NetvoxFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? NetvoxFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
void NetvoxFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void NetvoxFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &NetvoxFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &NetvoxFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
@implementation NetvoxFMStatement
